...
async function checkRest() {
    const { data, count } = await supabase.from('master_inventory').select('*', { count: 'exact' }).eq('is_restaurant_item', true);
    console.log('Restaurant items count:', count);
    if (data) console.log('Sample Restaurant item:', data[0]);
}
checkRest();
