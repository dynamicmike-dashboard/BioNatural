...
async function fixPrices() {
    console.log('💰 Fixing $0 prices in inventory...');
    const { error } = await supabase.from('master_inventory').update({ price: 250 }).eq('price', 0);
    if (error) console.error(error.message);
    else console.log('✅ Prices updated to $250 placeholder.');
}
fixPrices();
