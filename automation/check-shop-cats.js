...
async function checkShopCats() {
    const { data } = await supabase.from('master_inventory').select('category').eq('is_restaurant_item', false);
    const cats = [...new Set(data.map(d => d.category))];
    console.log('Shop Categories in DB:', cats);
}
checkShopCats();
